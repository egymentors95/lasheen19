...
class dev_skip_installment(models.Model):
    ...
    def action_send_request(self):
        ...
        if self.manager_id and self.manager_id.work_email:
            template_id = self.env.ref('dev_hr_loan.dev_skip_dep_manager_approvall').id
            mtp = self.env['mail.template']
            template = mtp.browse(template_id)
            if not template.exists():
                raise ValidationError(_('The specified mail template does not exist.'))
            template.send_mail(self.id, email_values={'email_to': self.manager_id.work_email}, force_send=True)
        self.state = 'request'
    ...
    def get_hr_manager_email(self):
        ...
        email_list = []
        if group_ids:
            employee_ids = self.env['hr.employee'].search([('user_id', 'in', group_ids.user_ids.ids)])
            email_list = [emp.work_email for emp in employee_ids if emp.work_email]
        return ','.join(email_list)

    def approve_skip_installment(self):
        email = self.get_hr_manager_email()
        if email:
            template = self.env.ref('dev_hr_loan.dev_skip_ins_hr_manager_requestt')
            if not template:
                raise ValidationError(_('The specified mail template does not exist.'))
            template.send_mail(
                self.id,
    ...
    def dep_reject_skip_installment(self):
        if self.employee_id.work_email:
            ...
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.dep_manager_reject_skip_installment')[1]
    ...
    def hr_reject_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.hr_manager_reject_skip_installment')[1]
    ...
    def confirm_skip_installment(self):
        ...
        if self.employee_id.work_email and self.hr_manager_id:
            ir_model_data = self.env['ir.model.data']
            template_id = ir_model_data._xmlid_lookup('dev_hr_loan.hr_manager_confirm_skip_installment')[1]
