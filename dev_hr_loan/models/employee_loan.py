...
class employee_loan(models.Model):
    ...
    def paid_loan(self):
        ...
        # 1) Credit Loan Principal
        lines.append((0, 0, {
            'account_id': self.loan_type_id.loan_account.id,     # FIXED
            'partner_id': self.employee_id.user_partner_id.id,
            'name': self.name,
            'credit': self.loan_amount,
        }))

        # 2) Credit Interest (if any)
        if self.interest_amount:
            lines.append((0, 0, {
                'account_id': self.loan_type_id.interest_account_id.id,  # FIXED name
                'partner_id': self.employee_id.user_partner_id.id,
                'name': f"{self.name} - Interest",
                'credit': self.interest_amount,
            }))

        # 3) Debit employee liability (use dedicated loan credit account)
        debit_total = self.loan_amount + (self.interest_amount or 0.0)

        lines.append((0, 0, {
            'account_id': self.loan_type_id.interest_account.id,  # FIXED
            'partner_id': self.employee_id.user_partner_id.id,
            'name': '/',
            'debit': debit_total,
        }))
