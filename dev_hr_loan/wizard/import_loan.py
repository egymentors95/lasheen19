...
class import_loan(models.TransientModel):
    ...
    def import_loan(self):
        ...
        if self.file_type == 'csv':
            ...
            workbook = load_workbook(BytesIO(file_datas))
            sheet = workbook.active
            lines = [[sheet.cell(row=r, column=c).value for c in range(1, sheet.max_column + 1)] for r in range(1, sheet.max_row + 1)]
            lines.pop(0)
        ...
        for line in lines:
            ...
            loan_type_id = self.get_loan_type(line[2])
            if not loan_type_id:
                remark = self.add_in_remark(remark,'Loan type')
            if not remark:
                loan_date = line[3]
                if self.file_type == 'excel':
                    loan_date = datetime(*xlrd.xldate_as_tuple(line[3], workbook.datemode)).date()
                res ={
