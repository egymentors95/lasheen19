...
class AttendanceSheet(models.Model):
    ...
    contract_id = fields.Many2one('hr.version', string='Employee Record',
                                  readonly=False)
    ...
    def _compute_diff_days(self):
        for sheet in self:
            ...
            days = 0
            if not contract_id:
                contracts_dict = sheet.employee_id._get_contracts(sheet.date_from,
                                                                  sheet.date_to)
                version = contracts_dict.get(sheet.employee_id.id)
                if version:
                    contract_id = version
            if not contract_id:
                sheet.no_diff_days = 0
                continue
            if not contract_id.contract_date_start:
                sheet.no_diff_days = 0
                continue
            if contract_id.contract_date_start <= sheet.date_from and not contract_id.contract_date_end:
                sheet.no_diff_days = 0

            if contract_id.contract_date_start <= sheet.date_from and contract_id.contract_date_end and contract_id.contract_date_end >= sheet.date_to:
                sheet.no_diff_days = 0
            elif contract_id.contract_date_start >= sheet.date_from and (
                    not contract_id.contract_date_end or (
                    contract_id.contract_date_end and contract_id.contract_date_end >= sheet.date_to)):
                sheet.no_diff_days = (contract_id.contract_date_start - sheet.date_from).days
            elif contract_id.contract_date_start <= sheet.date_from and contract_id.contract_date_end and contract_id.contract_date_end <= sheet.date_to:
                sheet.no_diff_days = (sheet.date_to - contract_id.contract_date_end).days

            if sheet.no_diff_days <= 0:
                if sheet.contract_id.contract_date_start and sheet.contract_id.contract_date_start > sheet.date_from \
                        and sheet.contract_id.contract_date_end and sheet.contract_id.contract_date_end < sheet.date_to:
                    sheet.no_diff_days = (sheet.contract_id.contract_date_start - sheet.date_from).days + (
                            sheet.date_to - sheet.contract_id.contract_date_end).days
                elif sheet.contract_id.contract_date_start and sheet.contract_id.contract_date_start > sheet.date_from:
                    sheet.no_diff_days = (sheet.contract_id.contract_date_start - sheet.date_from).days
                elif sheet.contract_id.contract_date_end and sheet.contract_id.contract_date_end < sheet.date_to:
                    sheet.no_diff_days = (sheet.date_to - sheet.contract_id.contract_date_end).days
    ...
    def _cron_generate_attendance_sheet(self):
        ...
        version_ids = self.env['hr.version'].search([
            ('auto_attendance_sheet', '=', True),
            ('att_policy_id', '!=', False),
            ('contract_date_start', '!=', False),
            ('contract_date_start', '<=', end_month),
            '|', ('contract_date_end', '=', False), ('contract_date_end', '>=', start_month),
        ])
        _logger.info("Found %s employee records eligible for attendance sheet generation.", len(version_ids))
    ...
    def create(self, vals_list):
        """Set name when not provided (same logic as onchange_employee)."""
    ...
    def onchange_employee(self):
        ...
        contracts_dict = employee._get_contracts(date_from, date_to)
        version = contracts_dict.get(employee.id)
        if not version:
    ...
    def _create_ab_lines(self, day, tz, policy_id, calendar_id, abs_cnt, leaves, wk_interval):
        ...
        contract_start = contract_id.contract_date_start
        contract_end = contract_id.contract_date_end
    ...
    def action_create_payslip(self):
        ...
        for sheet in self:
            ...
            payslip_name = version.structure_type_id.default_struct_id.payslip_name or _('Salary Slip')
